...
RUN apt update -y;
RUN echo "deb http://archive.ubuntu.com/ubuntu focal partner" | tee -a /etc/apt/sources.list;
RUN echo "deb-src http://archive.ubuntu.com/ubuntu focal partner" | tee -a /etc/apt/sources.list;
RUN apt install cloudcompare -y;
...
RUN cd /src/web \
    && yarn \
    && yarn build
RUN sed -i 's#app/locale/#novnc/app/locale/#' /src/web/static/novnc/app/ui.js
